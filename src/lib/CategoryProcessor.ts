...
class CategoryProcessor {
     ...
     */
    getCategories(source: Source): string {
        const rootCategories = this.categoryManager.getRootCategories();
        const lines: string[] = [];
        let matched = false;

        for (const rootCategory of rootCategories) {
            const result = this.processRecipeForRootCategory(source, rootCategory);
            if (!result.hasMatchingCategories) continue;

            matched = true;

            const format = (cat: any) =>
    ...
    private processRecipeForRootCategory(source: Source, rootCategory: CategoryDTO): RootCategoryResult {
        ...
        const leafScores = this.getScoresForCategories(source, leafCategories, this.leafScoreThreshold)
        ...
        if (!hasHighConfidenceLeaf && leafScores.length < this.minLeafCategories) {
            intermediateScores = this.getFilteredIntermediateScores(source, intermediateCategories, leafScores);
        }
    ...
    private getFilteredIntermediateScores(source: Source, intermediateCategories: CategoryDTO[], leafScores: CategoryScore[]): CategoryScore[] {
        // Get all intermediate scores first
        const allIntermediateScores = this.getScoresForCategories(source, intermediateCategories, this.intermediateScoreThreshold);
    ...
    private getScoresForCategories(source: Source, categories: CategoryDTO[], threshold: number): CategoryScore[] {
        if (categories.length === 0) return [];

        const { scores } = scoreRecipeCategories(source, categories);
